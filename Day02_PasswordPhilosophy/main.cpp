...
bool is_valid_password_part_one(const password_policy& policy, const string& password) {
    int count = count_c(policy.c, password);
    return policy.min_c <= count && count <= policy.max_c;
}

bool is_valid_password_part_two(const password_policy& policy, const string& password) {
...
int main() {
    ...
    for (size_t i = 0; i < passwords.size(); ++i) {
        if (is_valid_password_part_one(policies[i], passwords[i])) ++correct_part_one;
        if (is_valid_password_part_two(policies[i], passwords[i])) ++correct_part_two;
    }
